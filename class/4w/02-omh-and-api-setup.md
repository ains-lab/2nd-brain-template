...
저장소 루트 SCHEMA.md를 기준으로 raw/web, entities, concepts, comparisons,
queries, index.md, log.md 구조를 사용해. SCHEMA가 일반 스킬 예시보다 우선해.
...
스킬이 실제로 노출되지 않으면 로드했다고 주장하지 말고 설치 상태를 확인한다. 위 운영안이 준비됐다는 것과 파일이 생성됐다는 것은 다르다. 실제 반영은 [핸즈온](03-hands-on.md)에서 `wiki_pipeline.py`로 **격리된 빈 Wiki 초기화 → raw export → prepare**를 실행한 후 진행한다.
